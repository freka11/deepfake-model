...
def load_model():
    ...
    # Try to load multi-dataset model first, then fallback to old model
    model_paths = [
...
def predict_file(file_obj):
    ...
    if mime and mime.startswith("image"):
        ...
        with torch.no_grad():
            ...
            print(f"DEBUG: conf={conf.item():.3f}, pred={pred.item()}, threshold check: {conf.item() < 0.9}")
            if conf.item() < 0.9:
                pred = torch.tensor(1)  # Force to FAKE
                print("DEBUG: Forced to FAKE")
        ...
        with torch.no_grad():
            ...
            probs = torch.softmax(out, dim=1)[0]
            conf, pred = torch.max(probs, dim=0)
            print(f"DEBUG VIDEO: conf={conf.item():.3f}, pred={pred.item()}, threshold check: {conf.item() < 0.9}")
            if conf.item() < 0.9:
                pred = torch.tensor(1)  # Force to FAKE
                print("DEBUG VIDEO: Forced to FAKE")
